...
# ==========================================
# 1. CONFIGURAÇÃO DE LAYOUT E IDENTIDADE (CSS)
# ==========================================
...
st.markdown("""
    <style>
        ...
        /* Estilos do Prontuário Clínico Visual */
        .prontuario-card {
        ...
        .prontuario-header { text-align: center; border-bottom: 3px solid #2E5A44; padding-bottom: 10px; color: #2E5A44 !important; }
        .prontuario-secao { border-bottom: 1px solid #ccc; margin-top: 20px; padding-bottom: 5px; color: #2E5A44 !important; font-weight: bold; }
        .tabela-prontuario { width: 100%; border-collapse: collapse; margin-top: 10px; }
        .tabela-prontuario td { padding: 8px; border: 1px solid #ddd; color: #000000 !important; }
        
        @media print {
            [data-testid="stSidebar"], button, .no-print { display: none !important; }
            .prontuario-card { border: none !important; padding: 0 !important; }
        }
    </style>
""", unsafe_allow_html=True)

# ==========================================
# 2. FUNÇÕES AUXILIARES E CONVERSORES
# ==========================================
...
try:
    ...
    try: df_alunos = limpiar_dataframe(conn.read(worksheet="alunos", ttl=2))
    except: pass
    try: df_financeiro = limpiar_dataframe(conn.read(worksheet="financeiro", ttl=2))
    except: pass
    try: df_espera = limpiar_dataframe(conn.read(worksheet="espera", ttl=2))
    except: pass
    try: df_precos = limpiar_dataframe(conn.read(worksheet="precos", ttl=2))
    except: pass
    try: df_evolucoes = limpiar_dataframe(conn.read(worksheet="evolucao", ttl=2))
    except: pass
    conexao_ok = True
...
if not df_precos.empty and "Plano" in df_precos.columns:
    for _, r in df_precos.iterrows():
        dict_precos[str(r["Plano"])] = converter_para_float(r["Valor"])
...
with st.sidebar:
    ...
    st.markdown("---")
    if conexao_ok: st.success("● Banco de Dados Online")
    else: st.error("● Banco de Dados Offline")

LISTA_QUEIXAS_PADRAO = ["Dor Lombar (Lombalgia)", "Hérnia de Disco / Protrusão", "Dor / Lesão nos Ombros", "Dor Cervical (Cervicalgia)", "Dor / Lesão nos Joelhos", "Melhoria Postural Operacional", "Pilates para Gestantes", "Pilates para Terceira Idade (Idosos)", "Condicionamento Físico Geral"]

# ==========================================
# 5. DIRECIONAMENTO DAS TELAS
# ==========================================
...
# --- IMPRIMIR PRONTUÁRIO ---
    ...
    if not df_alunos.empty:
        ...
        if sel != "-- Escolha --":
            ...
            c_html = "Nenhuma cadastrada." if c_p.lower() == "nan" or not c_p.strip() else c_p

            # Correção crucial aqui: Usar uma única variável de string HTML bem formatada
            html_prontuario = (
                f'<div class="prontuario-card">'
                f'  <div class="prontuario-header">'
                f'      <h2>HIGHLINE STUDIO PILATES</h2>'
                f'      <p>Ficha Clínico-Funcional & Anamnese Estruturada</p>'
                f'  </div>'
                f'  <div class="prontuario-secao">1. DADOS IDENTIFICATÓRIOS</div>'
                f'  <table class="tabela-prontuario">'
                f'      <tr><td><strong>Nome Completo:</strong> {row.get("Nome","-")}</td><td><strong>Gênero:</strong> {row.get("Genero","-")}</td></tr>'
                f'      <tr><td><strong>Nascimento:</strong> {row.get("Nascimento","-")} ({idade} anos)</td><td><strong>CPF:</strong> {row.get("CPF","-")}</td></tr>'
                f'      <tr><td><strong>WhatsApp:</strong> {row.get("Telefone","-")}</td><td><strong>Início das Aulas:</strong> {row.get("Inicio_Aulas","-")}</td></tr>'
                f'      <tr><td colspan="2"><strong>Endereço:</strong> {row.get("Endereco","-")} | <strong>Bairro:</strong> {row.get("Bairro","-")}</td></tr>'
                f'  </table>'
                f'  <div class="prontuario-secao">2. ANAMNESE CLÍNICA (QUEIXAS PRINCIPAIS)</div>'
                f'  <p style="color:black; margin-top:10px;">● {q_html}</p>'
                f'  <div class="prontuario-secao">3. DIRETRIZES DE CONDUTA</div>'
                f'  <p style="color:black; margin-top:10px;">{c_html}</p>'
                f'</div>'
            )
